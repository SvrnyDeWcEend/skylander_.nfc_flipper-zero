...
# Created by SvrnyDeWcEend with code portions borrowed from equipter/ClassicConverter
...
def get_uid(contents: bytes) -> str:
    Block = []
    for i in range(4):  # Veranderd naar 4, zodat alleen de eerste 4 bytes worden genomen
        byte = contents[i : i + 1].hex()
        Block.append(byte)
...
def get_sak(contents: bytes) -> str:
    Block = []
    # Pas het bytebereik aan om twee bytes te selecteren voor SAK
    for i in range(7, 9):
...
def get_atqa(contents: bytes) -> str:
    Block = []
    # Pas het bytebereik aan om twee bytes te selecteren voor ATQA
    for i in range(8, 10):
